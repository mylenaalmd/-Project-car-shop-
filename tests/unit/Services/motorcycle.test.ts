...
import { motorcycleInput,
  ...
  motorcycleArrayOutPutId } from '../../motorcycle.mock';

describe('Teste para criação de uma moto', function () {
  it('Teste se é criada uma moto com sucesso', async function () {
  ...
  it('Teste se é retornado uma moto por um ID em formato incorreto', async function () {
    try {
      const service = new MotorcycleService();
      await service.findMotorcycleById('63776ded12654d2053a32382AA');
    } catch (error) {
  ...
  it('Teste se é atualizado uma moto por um ID em formato incorreto', async function () {
    try {
      const service = new MotorcycleService();
      await service.updateMotorcycle('63776ded12654d2053a32382AA', motorcycleInput);
    } catch (error) {
      expect((error as Error).message).to.be.deep.equal('Invalid Mongo id');
    }
  });
